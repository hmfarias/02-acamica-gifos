...
searchIcon.addEventListener("click", () => {
	if (searchInput.value === "") {
		arrSearch = []; // reinitialize the search array for slide en max mode for desktop
		searchPrepare();
	} else {
		searchInput.value = "";
		searchInput.focus();
...
async function showSearch(word, offset) {
	try {
		...
		if (info.length > 0) {
			...
			searchGif
				.querySelectorAll(".gifSearchContainer > img")
				.forEach((gifElement) => {
					...
					if (desktopDisplay.matches) {
						gifElement.addEventListener("mouseover", (event) => {
							let idHoverSearch =
								"divHoverSearch" + event.target.getAttribute("id");
			...
			searchGif
				.querySelectorAll(".divHoverSearch__icons > img")
				.forEach((icon) => {
					icon.key = icon.id;
...
function hideIlustraHeader() {
	...
	searchGif.innerHTML === 0 || searchGif.innerHTML === ""
		? (btnShowMore.style.display = "none")
		: (btnShowMore.style.display = "block");
...
function searchGifs() {
	if (searchInput.value !== "") {
		...
		searchSuggestion.style.display = "none";
		arrSearch = []; // reinitialize the search array fos slide en max mode for desktop
...
async function showTrending() {
	try {
		...
		//suscribe each Gif to click event in order to changge it to full screen mode
		trendingGif
			.querySelectorAll(".gifTrendingContainer > img")
			.forEach((gifElement) => {
				gifElement.addEventListener("click", clickOnGif, false);
...
async function clickOnGif(gif) {
	try {
		...
		placeInArray = arrActive.indexOf(gif.target.id);
		btnLeft.key = placeInArray;
		btnRight.key = placeInArray;
		btnLeft.array = arrActive;
...
async function previousGif(event) {
	...
	let placeInArray = null;
	placeInArray = arrActive.indexOf(prevId);
	btnLeft.key = placeInArray;
	btnRight.key = placeInArray;
	btnLeft.array = arrActive;
	btnRight.array = arrActive;
...
async function nextGif(event) {
	...
	let placeInArray = null;
	placeInArray = arrActive.indexOf(nextId);
	btnLeft.key = placeInArray;
	btnRight.key = placeInArray;
	btnLeft.array = arrActive;
	btnRight.array = arrActive;
...
function manageFavorite(gif) {
	...
	Object.prototype.toString.call(gif) === "[object MouseEvent]"
		? (gifID = gif.target.id)
		: (gifID = gif.id);
	...
	try {
		...
		info.forEach((element, index) => {
			...
			if (index < trendingLimitDescription) {
				paragraph =
					element + (index === trendingLimitDescription - 1 ? "" : ", ");
...
function loadFavorites() {
	...
	displayPrepare(
		[sectionTrending, sectionFavorites, navMenu],
		"block",
		[
			ilustraHeader,
			sectionSearch,
...
function loadMyGifs() {
	...
	//hide the sections that should not appear
	displayPrepare(
		[sectionTrending, sectionMyGifs, navMenu],
		"block",
		[
			ilustraHeader,
			sectionSearch,
...
function createGifStepTwo() {
	...
	stateUploadImg.style.animation =
		"rotateAxisX 3s linear 0s infinite normal backwards";
	...
	canvasVideo.style.animation = "rotateAxisY 1.5s linear 0s 1 normal backwards";
...
function saveGif(event) {
	...
	//show / hide  nodes
	displayPrepare(
		[btnEndGif, filmCrono, projectionLight],
		"block",
		[btnStartGif, btnSaveGif, btnUploadGif, filmRepeat],
		"none"
	);
	btnStartGif.style.visibility = "hidden";
	btnSaveGif.style.visibility = "hidden";
...
function createGifStepThree() {
	...
	gifUploaded
		.then((response) => {
			...
			stateUploadImg.style.animation =
				"rotateAxisX 3s linear 0s 2 normal backwards";
...
function endSaveGif() {
	...
	//show / hide  nodes
	displayPrepare(
		[btnUploadGif, filmRepeat],
		"block",
		[projectionLight, filmCrono, btnEndGif, btnSaveGif],
		"none"
	);
	btnStartGif.style.visibility = "hidden";
